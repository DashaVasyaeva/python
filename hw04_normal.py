...
#  2-ой способ
list_from_line = [i for i in line]
for i in list_from_line:
    if i.isupper():
        list_from_line[list_from_line.index(i)] = ' '
new_line = ''.join(list_from_line)
...
# Дальше идея была сделать с регулярными выражениями, типа
# print(re.search(r'([0-9])\1+', number).group(0))
# Но match и search не могу использовать, так как хотела найти все вхождения.
# А если сделать так:
# print(re.findall(r'([0-9])\1+', number))
# то не могу использовать метод group, он применим только к match и search, а без него выводится только цифра,
